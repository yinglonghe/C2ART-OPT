...
import c2art_env.sim_env.hybrid_mfc.defaults as defaults
...
def Armax(total_mass, car_power, road_type=1, car_type=1):
...
def ev_curves(
        ...
        f2):
    phi = car.phi
    motor_max_power = car.motor_max_power  # kW
    ...
    Alimit = Armax(veh_mass, motor_max_power, car_type=car_type)
    Res = []
    tract_acc = cs_acc_ev[0](veh_speed)
    tract_acc[tract_acc > Alimit] = Alimit
    final_acc = tract_acc - car_res_curve(veh_speed)
    final_acc = final_acc / phi
    ### make 0 the acc outside gear possible use
    Start = []
    Stop = []
    Start.append(veh_speed[0])
    Stop.append(veh_speed[-1])
    final_acc[(veh_speed < Start)] = 0
    final_acc[(veh_speed > Stop)] = 0
    final_acc[final_acc < 0] = 0
    Res.append(interp1d(veh_speed, final_acc))
...
def gear_curves(
    ...
    Alimit = Armax(veh_mass, engine_max_power, car_type=car_type)
    Res = []
    speed_per_gear[0][0] = 0
    for gear, acc in enumerate(cs_acc_per_gear):
        tract_acc = acc(sp_bins)
        tract_acc[tract_acc > Alimit] = Alimit
        final_acc = tract_acc - car_res_curve(sp_bins)
        final_acc = final_acc / phi
        ### make 0 the acc outside gear possible use
...
def hybrid_curves(
    ...
    for j in range(len(gr)):
        ...
        for i in range(len(motor_full_load_speeds)):
            motor_speed_per_gear[j].append(
                2 * np.pi * tire_radius * motor_full_load_speeds[i] * (1 - driveline_slippage) / (60 * final_drive * gr[j])
            )
            motor_acc_per_gear[j].append(
                motor_full_load_torque[i] * (final_drive * gr[j]) * driveline_efficiency / (tire_radius * veh_mass)
            )
    ...
    for j in range(len(gr)):
        ...
        cs_motor_acc_per_gear.append(
            CubicSpline(
                ...
                [motor_acc_per_gear[j][0]] * len(prefix_list) + motor_acc_per_gear[j] + [motor_acc_per_gear[j][-1]] * len(suffix_list)
            )
        )
    ...
    sp_bins = np.arange(0,top_speed+0.1,0.1)
    car_res_curve, car_res_curve_force = veh_resistances(f0, f1, f2, list(sp_bins), veh_mass)
    Alimit = Armax(veh_mass, max_power, car_type=car_type)
    Res = []
    for i in range(len(gr)):
        ice_acc = cs_acc_per_gear[i](sp_bins)
        start = speed_per_gear[i][0]
        stop = speed_per_gear[i][-1]
        ice_acc[(sp_bins < start)] = 0
        ice_acc[(sp_bins > stop)] = 0

        em_acc = cs_motor_acc_per_gear[i](sp_bins)
        start = motor_speed_per_gear[i][0]
        stop = motor_speed_per_gear[i][-1]
