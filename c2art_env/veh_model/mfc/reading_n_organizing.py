import c2art_env.veh_model.mfc.vehicle_specs_class as vcc
...
def get_vehicle_from_db(db_dict, car_id, **kwargs):
    ...
    if my_car["Drive-Drive system"] == 'electric engine':
        electric = 1
    else:
        electric = 0

    my_car_specs = vcc.veh_specs(my_car, electric=electric, **kwargs)

    return my_car_specs


# def get_vehicle_from_db_raw(db_dict, car_id, **kwargs):
#     '''
#
#     kwargs can be:
#     lco = True          # Light co2mpas is to be used, so the relevant parameters must be imported
