...
# %% Define functions
def data_preprocess(data_path, Dataset, hyd_mode, res_path):

    df = pd.read_csv(os.path.join(data_path, Dataset), \
        usecols=['Time [s]', 'Current speed [km/h]', 'EngineRPM [rpm]', 'StateOfChargeBMS [%]', 'PedalPosition_1 %'])
    df['Time [s]'] = df['Time [s]'] - df['Time [s]'][0]
    df['Speed [m/s]'] = df['Current speed [km/h]'] / 3.6
    df['Accel [m/s2]'] = np.append((utils.numbadiff(df['Speed [m/s]'].to_numpy()) / utils.numbadiff(df['Time [s]'].to_numpy())), 0)
    
    t = np.round(np.arange(0, round(df['Time [s]'].iloc[-1]*10+1), 1)*0.1, 1)
    dfraw, dfnew = pd.DataFrame(), pd.DataFrame()
    dfraw['Time [s]'], dfnew['Time [s]'] = t, t
    ls_col = ['Speed [m/s]', 'EngineRPM [rpm]', 'StateOfChargeBMS [%]', 'PedalPosition_1 %']
    for col in ls_col:
        f = interpolate.interp1d(df['Time [s]'], df[col], fill_value='extrapolate')
        yraw = f(t)
        if col == 'Speed [m/s]':
            yraw = np.maximum(yraw, 0)
            ynew = signal.savgol_filter(yraw, 51, 3)
            ynew = np.maximum(ynew, 0)
        elif col == 'StateOfChargeBMS [%]':
            ynew = signal.savgol_filter(yraw, 151, 1)
        else:
            ynew = yraw
        dfraw[col] = yraw
        dfnew[col] = ynew
    dfraw['Accel [m/s2]'] = np.append((utils.numbadiff(dfraw['Speed [m/s]'].to_numpy()) / utils.numbadiff(dfraw['Time [s]'].to_numpy())), 0)
    dfnew['Accel [m/s2]'] = np.append((utils.numbadiff(dfnew['Speed [m/s]'].to_numpy()) / utils.numbadiff(dfnew['Time [s]'].to_numpy())), 0)

    if hyd_mode == 'CD':
        dfraw = dfraw.loc[dfraw['Time [s]']<=2040]
        dfnew = dfnew.loc[dfnew['Time [s]']<=2040]
    dfraw.to_csv(os.path.join(res_path, 'Interp_'+hyd_mode+'.csv'), index=False)
    dfnew.to_csv(os.path.join(res_path, 'Interp_Smooth_'+hyd_mode+'.csv'), index=False)

    f, axs = plt.subplots(len(ls_col)+1, 1, figsize=(10,8), sharex=True)
    for i, col in enumerate(ls_col + ['Accel [m/s2]']):
        axs[i].plot(df['Time [s]'], df[col], label='Exp')
        axs[i].plot(dfraw['Time [s]'], dfraw[col], 'r--', label='Interp')
        axs[i].plot(dfnew['Time [s]'], dfnew[col], label='Interp+Smooth')
        axs[i].set_ylabel(col)
        axs[i].grid()
        axs[i].legend()
    plt.savefig(os.path.join(res_path, 'InterpSmooth_'+hyd_mode+'.png'))
    # plt.show()
...
if __name__ == "__main__":
    car_id = 26712  # Niro, https://www.cars-data.com/en/kia-niro-1.6-gdi-hybrid-executiveline-specs/76106
    ...
    data_path = os.path.join(pathRoot, 'c2art_env', 'sim_env', 'hybrid_mfc',
                        'datasets', 'kia_niro')
    trajDataset = [
        '20190424_02_WarmUpCD_ElasticityCD_SailingCD_ConstSpeedCS_aligned.csv',
        '20190424_03_ElasticityCS_SailingCS_aligned.csv']

    hyd_mode = 'CD'
    res_path = os.path.join(pathRoot, 'PostProcessing', 'HybridMFC', 'Outputs', car.model, hyd_mode, 'traj_cal_val')
    if not os.path.exists(res_path):
        os.makedirs(res_path, exist_ok=True)
    df_cd = data_preprocess(data_path, trajDataset[0], hyd_mode, res_path)
    df_cd_a = df_cd.loc[(df_cd['Time [s]']>=1310) & (df_cd['Time [s]']<=2010)]
    plt_driving_traj(df_cd_a, os.path.join(res_path, 'traj_CD_a.png'))
    df_cd_b = df_cd.loc[(df_cd['Time [s]']>=720) & (df_cd['Time [s]']<=920)]
    plt_driving_traj(df_cd_b, os.path.join(res_path, 'traj_CD_b.png'))
    ...
    if not os.path.exists(res_path):
        os.makedirs(res_path, exist_ok=True)
    df_cs = data_preprocess(data_path, trajDataset[1], hyd_mode, res_path)
    df_cs_a = df_cs.loc[(df_cs['Time [s]']>=1160) & (df_cs['Time [s]']<=1790)]
    plt_driving_traj(df_cs_a, os.path.join(res_path, 'traj_CS_a.png'))
    df_cs_b = df_cs.loc[(df_cs['Time [s]']>=850) & (df_cs['Time [s]']<=1115)]
    plt_driving_traj(df_cs_b, os.path.join(res_path, 'traj_CS_b.png'))
